  ...
  try {
    ...
    const prompt = `
      ...
      **GEO & SEO GUIDELINES (STRICT)**:
      1. **ABSOLUTE ANTI-AI CONSTRAINT (CRITICAL)**: 제목, 개요, 본문 원고 전체에서 'AI', '인공지능', '챗봇', 'ChatGPT', 'AI 기술' 등 AI 관련 용어 언급 및 단어 노출을 **절대 전면 금지**합니다. 설령 블로그의 메인 키워드나 주제 자체에 'AI'나 '인공지능'이라는 단어가 직접 포함되어 있더라도, 본문이나 제목, 개요 원고 내에서는 이 직접적인 단어를 일절 노출하지 않고 '스마트', '첨단 기술', '자동화 기술', '최첨단 시스템' 등의 대체어로 우회하여 표현해야만 합니다. 어떠한 경우에도 글에서 'AI' 혹은 '인공지능'이라는 단어가 노출되어서는 안 되며, AI에 기인했음을 나타내는 표현을 모두 배제하고 100% 사람이 직접 작성한 것처럼 완성형 글로 표현해야 합니다.
  ...
  try {
    ...
    const promptText = `
      ...
      **UNIVERSAL BLOG STYLE GUIDELINES (MUST FOLLOW FOR ALL TOPICS)**:
      ...
      12. **ABSOLUTE ANTI-AI CONSTRAINT (CRITICAL)**: 제목, 개요, 본문 원고 전체에서 'AI', '인공지능', '챗봇', 'ChatGPT', 'AI 기술' 등 AI 관련 용어 언급 및 단어 노출을 **절대 전면 금지**합니다. 설령 블로그의 메인 키워드나 주제 자체에 'AI'나 '인공지능'이라는 단어가 직접 포함되어 있더라도, 본문이나 제목, 개요 원고 내에서는 이 직접적인 단어를 일절 노출하지 않고 '스마트', '첨단 기술', '자동화 기술', '최첨단 시스템' 등의 대체어로 우회하여 표현해야만 합니다. 어떠한 경우에도 글에서 'AI' 혹은 '인공지능'이라는 단어가 노출되어서는 안 되며, AI에 기인했음을 나타내는 표현을 모두 배제하고 100% 사람이 직접 작성한 것처럼 완성형 글로 표현해야 합니다.
  ...
  try {
    ...
    const prompt = `
      ...
      **READABILITY (Spacing - CRITICAL)**: 
      ...
      - **ABSOLUTE ANTI-AI CONSTRAINT (CRITICAL)**: 제목, 개요, 본문 원고 전체에서 'AI', '인공지능', '챗봇', 'ChatGPT', 'AI 기술' 등 AI 관련 용어 언급 및 단어 노출을 **절대 전면 금지**합니다. 설령 블로그의 메인 키워드나 주제 자체에 'AI'나 '인공지능'이라는 단어가 직접 포함되어 있더라도, 본문이나 제목, 개요 원고 내에서는 이 직접적인 단어를 일절 노출하지 않고 '스마트', '첨단 기술', '자동화 기술', '최첨단 시스템' 등의 대체어로 우회하여 표현해야만 합니다. 어떠한 경우에도 글에서 'AI' 혹은 '인공지능'이라는 단어가 노출되어서는 안 되며, AI에 기인했음을 나타내는 표현을 모두 배제하고 100% 사람이 직접 작성한 것처럼 완성형 글로 표현해야 합니다.
